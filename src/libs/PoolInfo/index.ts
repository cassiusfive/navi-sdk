...
export const fetchPoolData = async ({
    ...
    const total_supply_with_index =
        (poolData.total_supply * filedsData.current_supply_index) / 1e27;
    const total_borrow_with_index =
        (poolData.total_borrow * filedsData.current_borrow_index) / 1e27;
    ...
    return {
        ...
        current_supply_utilization:
            total_supply_with_index /
            Number(filedsData.supply_cap_ceiling / 1e36),
        current_borrow_utilization:
            total_borrow_with_index /
            (Number((filedsData.borrow_cap_ceiling / 1e27).toFixed(2)) *
                poolData.total_supply),
        optimal_borrow_utilization: (
            Number(
                filedsData.borrow_rate_factors?.fields?.optimal_utilization,
            ) / 1e27
        ).toFixed(2),
        pool: poolData.pool,
        max_ltv: (Number(filedsData.ltv) / 1e27).toFixed(2),
        liquidation_threshold: (
            Number(filedsData.liquidation_factors.fields.threshold) / 1e27
        ).toFixed(2),
        ...
        if (coin) {
            ...
            for (const poolId in poolInfo) {
                if (poolInfo.hasOwnProperty(poolId)) {
                    poolResults[poolId] = await fetchPoolData({
                        poolId,
                        reserveParentId: config.ReserveParentId,
                        client,
                        poolInfo,
    ...
    const fetchChunk = async (chunk: string[]): Promise<CoinPrice[] | null> => {
        const attemptFetch = async (
            ...
            try {
                ...
                if (retries <= 0) {
                    console.error(
                        "Failed to fetch coin prices after multiple attempts:",
                        error,
                    );
                    return null;
                }
                console.warn(
                    `Attempt failed for chunk, retrying... (${maxRetries - retries + 1}/${maxRetries})`,
                );
...
export async function getAllPools() {
    try {
        ...
        throw new Error(
            `Failed to fetch pool data: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
    }
}
